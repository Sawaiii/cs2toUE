...
param(
    ...
    [switch]$Publish,           # also publish a GitHub release with both files
    [string]$Notes = "",        # release notes (default: generated by GitHub)
...
if (-not $Publish) {
    Write-Host ""
    Write-Host "   publish a release with:  build.ps1 -Publish"
    exit 0
}

# the version in the code is the single source of truth for the tag
$verLine = Select-String -Path (Join-Path $root 'cs2toue\__init__.py') -Pattern '__version__ = "(.+)"'
if (-not $verLine) { Die "could not read __version__ from cs2toue\__init__.py" }
$version = $verLine.Matches[0].Groups[1].Value
$tag = "v$version"

$gh = (Get-Command gh -ErrorAction SilentlyContinue).Source
if (-not $gh) { $gh = Join-Path $ws 'tools\gh\bin\gh.exe' }
if (-not (Test-Path $gh)) { Die "GitHub CLI not found - install it or put it in workspace\tools\gh" }

# the in-app updater downloads this one, so its name must stay exactly cs2toUE-app.zip
$appZip = Join-Path $root 'cs2toUE-app.zip'
Copy-Item $zip $appZip -Force

Say "publishing $tag"
& $gh release view $tag *> $null
if ($LASTEXITCODE -eq 0) {
    Say "release $tag already exists - replacing its files"
    & $gh release upload $tag $setup $appZip --clobber
} elseif ($Notes) {
    & $gh release create $tag $setup $appZip --title "cs2toUE $version" --notes $Notes
} else {
    & $gh release create $tag $setup $appZip --title "cs2toUE $version" --generate-notes
}
$rc = $LASTEXITCODE
Remove-Item $appZip -Force
if ($rc -ne 0) { Die "publishing failed" }
Ok "released: https://github.com/Sawaiii/cs2toUE/releases/tag/$tag"
Write-Host "   installed copies will offer this version within a day"
